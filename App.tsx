...
const App: React.FC = () => {
  ...
  const handleCreate = useCallback((promptString: string) => {
    const fallback: Breadcrumb = { sitename: siteNameFromPrompt(promptString), page: 'Home' };
    generate(promptString, null, fallback, true);
  }, [generate]);

  const handleLinkClick = useCallback((href: string, linkText: string, formState?: FormFieldState[]) => {
    const promptString = `User clicked "${linkText}" (href: ${href})`;
    ...
    if (isExternal) {
      ...
      generate(promptString, null, fallback, true, formState);
    } else {
      ...
      if (currentPage) {
        generate(promptString, currentPage.html, fallback, true, formState);
      } else {
        generate(promptString, null, fallback, true, formState);
      }
    }
  ...
  const handleOmnibarNavigate = useCallback((type: 'create' | 'edit', promptString: string) => {
    if (type === 'create') {
      const fallback: Breadcrumb = { sitename: promptString, page: 'Home' };
      generate(promptString, null, fallback, true);
    } else {
      if (!currentPage) return;
      const fallback: Breadcrumb = { sitename: activeTab.breadcrumb.sitename, page: promptString };
      generate(promptString, currentPage.html, fallback, false);
    }
  }, [generate, currentPage, activeTab.breadcrumb]);
